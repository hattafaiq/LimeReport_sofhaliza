...
void DataSourceManager::add_timing(float t_EC, float t_EO, float t_IC, float t_IO, float t_sil)
{
     timming_EC=t_EC;
     timming_EO=t_EO;
     timming_IC=t_IC;
     timming_IO=t_IO;
     timming_Silinder=t_sil;
     qDebug()<<"add timing->"<<timming_EC
            <<timming_EO
            <<timming_IC
            <<timming_IO
            <<timming_Silinder;
...
void DataSourceManager::add_peak(QVector<float> peak, int odd_even,QString peak_judul,QString suhu_judul, QVector<float> suhu, QStringList silinder,
    ...
    for(int i=0; i<peak.size(); i++){
     ...
     peak_igns = peak_ign;
     derajat_igns = derajat_ign;
     satuan_igns = satuan_ign;
